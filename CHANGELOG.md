...
- The whole crate, in favour of `bevy_rand` + `bevy_prng`. `StdRng` is documented
  as non-portable, so seeded reproducibility could not be guaranteed across `rand`
  releases, and the RNG state was never reflected. See the README for the mapping.
