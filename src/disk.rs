...
impl crate::GlobalRng {
    /// Uniform offset inside a disk of the given radius.
    ///
    /// Convenience wrapper around [`DiskSample::disk_offset`].
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    /// assert!(offset.length() <= 50.0 + f32::EPSILON);
    /// ```
    pub fn disk_offset(&mut self, radius: f32) -> Vec2 {
        self.inner().disk_offset(radius)
    }

    /// Offset shaped by an easing function to control density falloff.
    ///
    /// Convenience wrapper around [`DiskSample::disk_offset_eased`]; see it
    /// for the density and clamping rules.
    pub fn disk_offset_eased(&mut self, radius: f32, ease: EaseFunction) -> Vec2 {
        self.inner().disk_offset_eased(radius, ease)
    }
}
...
mod tests {
    ...
    fn disk_offset_eased_undershooting_ease_stays_finite() {
        let mut rng = crate::GlobalRng::seeded(42);
        let radius = 50.0;
        for _ in 0..1000 {
            let offset = rng.inner().disk_offset_eased(radius, EaseFunction::BackIn);
            assert!(
